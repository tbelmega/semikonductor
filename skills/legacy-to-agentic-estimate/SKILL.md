...
> **Capability maturity is the dominant lever, not the converted number.** Every
> mature, evaluator-backed capability you ship raises realized leverage (`L_eff`) and lowers the
...
tier. The fourth case shows that divergence coming purely from capability maturity.
...
- Don't accept an item with no description. Raise or ask rather than defaulting a
  tier.
